...
1. **Not Readability + Turndown.** The clipper migrated to [defuddle](https://github.com/kepano/defuddle) (kepano's own extractor). `defuddle@0.19.3`, MIT, published 2026-08-22. *(The original note here called `dist/index.full.js` a droppable 760 KB UMD bundle. In the event it is consumed from npm and bundled by `jsbridge/build.mjs` along with everything else, because Layer B imports `defuddle/full` directly; it is ~740 KB of the shipped bundle.)* One `parse()` with `separateMarkdown: true` returns clean HTML, markdown, metadata (title/author/published/description/domain/image/wordCount) and the clipper's template variables.
...
3. **The reader is one call on a plain Document.** `src/reader-script.ts` shows the entire integration is `Reader.toggle(document)`. Coupling is shallow: `reader.ts` is 2805 lines with only **18** `browser.*` references; `highlighter.ts` 1438 lines / 18 refs; `highlighter-overlays.ts` 743 lines / **0** refs. They were expected to funnel through three aliasable modules (`browser-polyfill`, `storage-utils`, `i18n`). **Measured at G0/B1: it is one.** Every `browser.*` reference reaches `webextension-polyfill` through `src/utils/browser-polyfill.ts` and nothing bypasses it, so a single esbuild alias replaces the whole extension API — the exact pattern `build-api.mjs` already uses with `cli-stubs.ts`.
...
| **B. Reader + highlighter** — the UI in screenshots 1 & 2 | `reader.ts`, `highlighter.ts`, `highlighter-overlays.ts`, `reader.scss`, `highlighter.scss` | **Vendored + shimmed.** ~5000 lines, ~36 API touchpoints, all behind **one** aliased module (measured, G0/B1). |
...
## What we are not: an extension

The single most consequential thing about this architecture, and the one that cost the most to
discover (G0/Spike B — measurements in Playbook §2). An extension's capabilities are not a library it
imports; they are privileges the browser grants it for *being* an extension:

| Extension has | We have | Consequence |
|---|---|---|
| An **isolated world** — same DOM, separate JS globals, not policed by the page's CSP or Trusted Types | The page's **main world**, fully policed | Pages sending `require-trusted-types-for 'script'` (YouTube) break extraction outright until we install a default policy — **D21** |
| A **`chrome-extension://` origin** whose `web_accessible_resources` are allowlisted past page CSP | Blob URLs, which `style-src` treats like any other URL | github.com refuses the reader's stylesheet, leaving the page stripped and unstyled — fixed by inlining, **D20** |
| **`<all_urls>` host permissions**, so its background worker fetches cross-origin freely | No CORS exemption | Rules out upstream's standalone-reader path without a Kotlin-side fetcher (see Playbook §6, GATE G0) |
| The browser **injecting content scripts** per frame at `document_idle` | `onPageFinished` on the main frame | SPA hydration timing is ours to solve — see the M1.6 note in Playbook §2 |

Android's WebView has no extension mechanism at all, so none of this is an architecture choice: no
shell design, however thin, obtains these privileges. **All four gaps are closed or bounded as of
G0** — the point of recording them is that each fix looks arbitrary until you know which privilege it
replaces, and a future session must not "simplify" one away.

The one thing that *does* come free: `evaluateJavascript` is neither a script element nor a
page-initiated `eval`, so our own bundle is never subject to `script-src`. Only the resources the
reader itself injects are.
...
- ~~A "Sign in to this site" action that drops out of reader mode into the live page with normal browser chrome.~~ **Ruled out by D23 (2026-08-31): no browser UI, ever** — no URL bar, back/forward, tabs, history or downloads. At most a "show the page as-is" toggle so an on-page login form is usable; a login-walled site that needs more than that is Johan's workaround to perform elsewhere, or a clip that does not happen.
